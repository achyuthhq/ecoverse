...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      ...
      // Handle error in URL by redirecting to dashboard if it's an account linking error
      if (url.includes('error=OAuthAccountNotLinked')) {
        console.log("Detected OAuthAccountNotLinked error, redirecting to dashboard");
    ...
    async signIn({ user, account, profile, email, credentials }) {
      // Allow sign in if using credentials or email provider
