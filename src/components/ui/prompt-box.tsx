...
>(({ className, sideOffset = 4, showArrow = false, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        "relative z-50 max-w-[280px] rounded-md glass-card border border-white/10 text-white px-1.5 py-1 text-xs animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <div className="relative glass-card rounded-[28px] overflow-hidden shadow-2xl p-1">
        {children}
        <DialogPrimitive.Close className="absolute right-3 top-3 z-10 rounded-full glass-card p-1 hover:bg-white/10 transition-all">
...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, PromptBoxProps>(
  ({ className, onSend, onVoiceRecord, selectedModel = 'openai', onModelChange, value: controlledValue, ...props }, ref) => {
    const internalTextareaRef = React.useRef<HTMLTextAreaElement>(null);
    const fileInputRef = React.useRef<HTMLInputElement>(null);
    const [internalValue, setInternalValue] = React.useState("");
    const [imagePreview, setImagePreview] = React.useState<string | null>(null);
    const [isImageDialogOpen, setIsImageDialogOpen] = React.useState(false);
    const [currentModel, setCurrentModel] = React.useState<string>(selectedModel);
    ...
    const handleSend = () => {
      // Allow sending if there's text (image is optional)
      if (String(currentValue).trim()) {
        if (onSend) {
          ...
          const apiModel = currentModel === 'openai-gpt5' ? 'openai' : currentModel;
          onSend(String(currentValue), apiModel);
        }
        if (controlledValue === undefined) {
          setInternalValue("");
        }
    ...
    // Enable send button if there's text (image is optional)
    const hasValue = String(currentValue).trim().length > 0;

    return (
      <div className={cn("flex flex-col rounded-[28px] p-2 shadow-sm transition-colors glass-card border border-white/10 cursor-text", className)}>
          ...
          <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
            <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1">
              ...
              <button
                ...
                className="absolute right-2 top-2 z-10 flex h-4 w-4 items-center justify-center rounded-full glass-card text-white transition-colors hover:bg-white/20"
        ...
        <textarea
          ...
          className="custom-scrollbar w-full resize-none border-0 bg-transparent p-3 text-white placeholder:text-gray-400 focus:ring-0 focus-visible:outline-none min-h-12 text-sm font-montserrat"
          {...props}
        />
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    type="button"
                    className="flex h-8 items-center gap-1.5 px-2.5 rounded-lg text-white text-xs font-medium transition-colors hover:bg-white/10 focus-visible:outline-none border border-white/10 glass-card"
                  >
                    <span className="font-montserrat">{modelOptions[currentModel as keyof typeof modelOptions] || 'GPT-4o'}</span>
                    <ChevronDown className="h-3 w-3 opacity-70" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent 
                  align="start" 
                  className="glass-card border border-white/10 min-w-[180px] p-1"
                >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      disabled={!hasValue}
                      className={cn(
                        "flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                        hasValue 
                          ? "bg-white text-gray-900 hover:bg-gray-100 cursor-pointer shadow-sm" 
                          : "bg-white/20 text-white/60 cursor-not-allowed border border-white/10"
                      )}
                    >
                      <SendIcon className={cn("h-5 w-5", hasValue ? "text-gray-900" : "text-white/60")} />
                      <span className="sr-only">Send message</span>
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top" showArrow={true}>
                    <p>{hasValue ? "Send" : "Type a message to send"}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
            </div>
