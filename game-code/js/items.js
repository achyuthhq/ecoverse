...
items.init = function() {
  ...
  items.defs[6]  = {name:"Green Glass",     x:243, y:12, w:17, h:50, rtype:items.recycle_types.GLASS};
  items.defs[7]  = {name:"Mason Jar",       x:262, y:27, w:23, h:35, rtype:items.recycle_types.GLASS};
  items.defs[8]  = {name:"Brown Glass",     x:287, y:13, w:16, h:49, rtype:items.recycle_types.GLASS};  
  ...
  items.defs[13] = {name:"Too Greasy Box",  x:403, y:32, w:60, h:30, rtype:items.recycle_types.LANDFILL};
  items.defs[14] = {name:"Coffee Mug",      x:465, y:36, w:29, h:26, rtype:items.recycle_types.LANDFILL};
  items.defs[15] = {name:"Pressurized Can", x:496, y:22, w:17, h:40, rtype:items.recycle_types.LANDFILL};
...
items.add_item = function(item_id) {
  ...
  // In GATE mode, place items statically on the conveyor (not moving)
  if (gamestate.current_state == gamestate.state_types.GATE) {
    // Place item in center of conveyor, static
    new_item.x = conveyor.left + 50;    
    new_item.y = conveyor.top - items.defs[new_item.itype].h;
    new_item.dx = 0;  // No movement
    new_item.dy = 0;
  } else {
    // Normal mode: spawn from right edge
...
items.move = function() {
  ...
  // In GATE mode, items don't move on their own (only when grabbed)
  var is_gate_mode = (gamestate.current_state == gamestate.state_types.GATE);
  ...
  for (var i=0; i < items.ilist.length; i++) {
    ...
    if (items.grabbing && i == items.grabbed_item) {
      ...
      // In gate mode, items stay static (no movement)
      if (is_gate_mode) {
        continue;
      }
...
items.release_check = function() {
  ...
  // In GATE mode, check if item is over a bin and collect immediately
  if (gamestate.current_state == gamestate.state_types.GATE) {
    var item_x = items.ilist[id].x + (items.defs[items.ilist[id].itype].w / 2);
    var item_y = items.ilist[id].y; // Use top y, not center (matches original toss logic)
    var target_bin = -1;
    
    // Check landfill bin first (it's at the bottom, needs special handling)
    // Landfill is on the left side, below the conveyor
    if (item_x < conveyor.left && item_y > 150) {
      target_bin = items.recycle_types.LANDFILL;
    } else {
      // Check the top 4 recycle bins (PLASTIC, PAPER, GLASS, METAL)
      for (var i=0; i<4; i++) {
        if (utils.is_within({x:item_x, y:item_y}, items.bins[i].hotspot)) {
          target_bin = i;
          break;
        }
      }
    ...
    if (target_bin != -1) {
      // Store item type before removing
      var item_type = items.ilist[id].itype;
      // Remove item FIRST to prevent any other logic from touching it
      items.remove(id);
      items.grabbing = false;
      // Then verify (this increments gate_recycles)
      scorekeeper.verify(item_type, target_bin);
      return;
    }
  }
  ...
  // Normal mode: check tossing item upwards into bin
  items.toss();  
...
items.collect = function() {
  ...
  // In GATE mode, collection is handled in release_check, skip this
  if (gamestate.current_state == gamestate.state_types.GATE) {
    return;
  }
