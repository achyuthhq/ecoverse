...
scorekeeper.verify = function(item_type, bin_type) {
  ...
  // In GATE mode, count all correct placements (including landfill)
  if (gamestate.current_state == gamestate.state_types.GATE && correct) {
    if (bin_type != items.recycle_types.LANDFILL) {
      scorekeeper.activate_symbol(scorekeeper.symbol_types.RECYCLE, bin_type);
    }
    // Track gate recycles for all correct placements
    gamestate.gate_recycles++;
    // Spawn a new static item immediately (no delay needed)
    ...
    return; // Early return for gate mode
  }
  ...
  if (correct && bin_type != items.recycle_types.LANDFILL) {
    ...
    // speed up the item flow (and difficulty) - only in PLAY mode
  ...
  if (!correct) {
    ...
    // Don't count mistakes in gate mode (just let them try again)
